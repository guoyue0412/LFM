...
- 3660 当前生成环境和 2026-07-27 repair 任务实际使用的 QBlade 子模块提交：`c042826847dfe2ff1a3c708e5b3acbf3d987e972`。历史 pkl 本身没有保存子模块提交；其写入时间晚于该提交且基准几何文件与该版本一致，因此历史版本归属是有证据支持的推断，不是 pkl 内直接记录的事实。
...
- 初始独立工作目录的 superproject gitlink 曾指向 `bbaff14`，而 3660 历史生成环境为 `c042826`。两者修改了基准桨文件、仿真对象和自由尾迹长度参数，不能混合作为同一数据版本。正式批次必须把 gitlink 固定到 `c042826`，并在每个新 pkl 中同时保存、核验 superproject 与 QBlade 子模块的完整提交哈希。
...
- 原始 pkl 交叉检查给出同一工况的 `Thrust_z=+0.203244 N` 与持久化 alias `THRUST_Z=-0.203244 N`，同时 `Thrust=-1.858040 N` 与 `THRUST=+1.858040 N`。这证明负 `H` 不是 CSV 聚合偶然误差，而是当前 wrapper 的显式带符号坐标约定。
